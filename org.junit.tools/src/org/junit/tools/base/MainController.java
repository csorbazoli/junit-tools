...
public class MainController implements IGeneratorConstants {
    ...
    private boolean generateTestclass(
        ...
        try {
            ...
            if (isError()) {
                if (errorException instanceof JUTWarning || errorException instanceof JUTException) {
            ...
            openInEditor(activeWorkbenchWindow.getShell(), (IFile) getGeneratedTestClass().getResource(),
                    getGeneratedTestMethod());

        } catch (JUTException | JUTWarning ex) {
            throw ex;
        } catch (Exception ex) {
            throw new JUTException(ex);
        }
    ...
    protected void setError(boolean error, Exception ex) {
        logger.severe("Error captured: " + ex);
        setError(error);
        errorException = ex;
    }
    ...
    protected void setGeneratedTestClass(ICompilationUnit generatedClass) {
        generatedTestClass = generatedClass;
    }
    ...
    protected void setGeneratedTestMethod(IMethod generatedMethod) {
        generatedTestMethod = generatedMethod;
    }
    ...
    /**
     * @param springTest TODO @throws CoreException @throws JavaModelException
     *                   Detects all the necessary JUT-Elements: project, package
     *                   and class for the base and test. @return detected
     *                   JUT-Elements @throws JUTException @throws
     *                   JUTWarning @throws
     */
    private JUTElements detectJUTElements(IStructuredSelection selection, IFileEditorInput fileEditorInput,
            boolean springTest)
        ...
        if (cuList.size() > 0) {
            ...
            jutElements.setConstructorsAndMethods(
                    getConstructorsAndMethods(jutElements.getClassesAndPackages().getBaseClass()));
    ...
    private boolean switchClass(IWorkbenchWindow activeWorkbenchWindow, JUTElements jutElements)
        ...
        if (classToOpen != null && classToOpen.exists()) {
            ...
            if (selectedMethod != null) {
                IMethod existingMethod = GeneratorUtils
                        .findMethod(Arrays.asList(classToOpen.findPrimaryType().getMethods()), mr);
