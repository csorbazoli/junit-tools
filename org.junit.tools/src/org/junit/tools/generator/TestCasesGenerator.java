...
public class TestCasesGenerator {
    ...
    public void generateTestCases(GeneratorModel utmModel)
        ...
        for (IMethod method : utmModel.getMethodsToCreate()) {
            ...
            List<Assertion> assertions = createAssertionForResult(tmlMethod,
                    classesAndPackages.getBaseClass().getParent().getElementName(),
    ...
    private List<Assertion> createAssertionsForResultType(String resultType, String methodName, String packageName,
            String testClass) {
        ...
        if ("Boolean".equalsIgnoreCase(resultType)) {
            ...
            defaultAssertion.setValue("");
            createAssertionsForResultType(resultType.replaceFirst("^Optional<(.*)>$", "$1"), methodName, packageName,
                    testClass).stream()
                            .map(assertion -> {
            ...
            defaultAssertion.setType(JUTPreferences.getAssertTypeForFiles());
            defaultAssertion.setBase("{result}.getBody()");
            ...
            extraAssertion.setType(JUTPreferences.getAssertTypeForFiles());
            extraAssertion.setBaseType(resultType);
            extraAssertion.setBase("{result}");
    ...
    private String getTestResourcePath(String packageName, String testClass, String methodName) {
        ...
        return "\"" + testClass + "_" + methodName + ".json\"";
    }
