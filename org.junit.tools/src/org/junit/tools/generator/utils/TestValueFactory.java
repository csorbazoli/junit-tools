...
public class TestValueFactory {
	...
	try {
		...
		ret = Arrays.asList(
			getValueForTypeInner(getGenericParameterType(type, 0), name, stack));
...
//		ret = new JSONObject("{}");
	...
	try {
		...
		log.warning("Failed to create initial value for " + getTypeAsString(type) + ": No String or default constructor is available");
	    }
	}
	return ret;
    }
	...
	if (key == null) {
	    ...
	    Object value = getValueForTypeInner(getGenericParameterType(maptype, 1), name,
		    stack);
	...
	for (Field f : type.getDeclaredFields()) {
		...
		if (stack.contains(fieldRef)) { // repeating reference - loop in structure hierarchy?
			...
			if (curValue == null || f.getType().isPrimitive()) {
			    ...
			    if (list.isEmpty()) {
				Object listItem = getValueForTypeInner(
					getGenericParameterType(f.getGenericType(), 0),
					f.getName(), stack);
				if (listItem != null) {
				    list.add(listItem);
				}
	...
	if (setter != null || Modifier.isPublic(f.getModifiers())) {
		...
		log.warning("Failed to set initial value of " + type.getName() + "." + f.getName() + ": " + e.getMessage());
	    }
	}
    }
	...
	if (ret == null) {
	    log.warning("There is no generic parameter type with index " + idx + " available for " + getTypeAsString(type));
	}
	return ret;
    }
